...
impl yellowstone_vixen_core::Parser for InstructionParser {
    ...
    fn prefilter(&self) -> yellowstone_vixen_core::Prefilter {
        yellowstone_vixen_core::Prefilter::builder()
            .transaction_accounts([crate::LB_CLMM_ID])
            .build()
            .unwrap()
    }
    ...
    async fn parse(
        ...
        if ix_update.program.equals_ref(crate::LB_CLMM_ID) {
...
impl yellowstone_vixen_core::ProgramParser for InstructionParser {
    ...
    fn program_id(&self) -> yellowstone_vixen_core::Pubkey {
        crate::LB_CLMM_ID.to_bytes().into()
    }
}
...
pub enum LbClmmProgramIx {
    InitializeLbPair(InitializeLbPair, InitializeLbPairInstructionArgs),
    NotImplemented,
}
    ...
    pub(crate) fn parse_impl(
        ...
        match ix_discriminator {
            [248, 198, 158, 145, 225, 117, 135, 200] => {
                ...
                let ix_accounts = InitializeLbPair {
                    lb_pair: ix.accounts[0].0.into(),
                    bin_array_bitmap_extension: if ix.accounts[1].0.ne(crate::LB_CLMM_ID.as_array())
                    {
                ...
                let ix_args = InitializeLbPairInstructionArgs::try_from_slice(ix_data)?;

                println!("Parsed InitializeLbPair");
                println!("accounts : {:#?}", ix_accounts);
                println!("ix_data : {:#?}", ix_args);
                Ok(LbClmmProgramIx::InitializeLbPair(ix_accounts, ix_args))
            }

            _ => Ok(LbClmmProgramIx::NotImplemented),
        }
    }
}
